...
.PHONY: help install dev test eval-retrieval eval-generation eval-chat eval-recommendation eval-report lint typecheck contracts migrate fmt verify
...
eval-retrieval: ## Retrieval golden-set eval — reports recall@5/MRR, gates >=0.85
	cd $(BACKEND) && $(abspath $(VENV))/pytest tests/eval -q -s \
		-m "not generation and not chat and not recommendation and not report"
...
eval-recommendation: ## Recommendation golden-set eval — pipeline vs fakes; gates schema/decline/zero-fabricated
	cd $(BACKEND) && $(abspath $(VENV))/pytest tests/eval -q -s -m recommendation

eval-report: ## Report golden-set eval — fixed knowledge→report graph vs fakes; gates decline/citations/metadata
	cd $(BACKEND) && $(abspath $(VENV))/pytest tests/eval -q -s -m report
